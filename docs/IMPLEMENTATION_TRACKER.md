...
- Normal legal weekdays default to `worked`, weekends are locked, and generated French holidays take visual precedence when they overlap weekends.
...
| Fixed year scope (`2025`-`2027`) | Implemented | The selector and state sanitization both enforce this range. |
...
| Baseline legal weekdays prefilled as worked | Implemented | `getDefaultDayRecord()` returns `worked` for non-holiday, non-weekend weekdays. |
| Simplified day-status model | Implemented | The current editable statuses are `worked`, `half_day`, `vacation`, `closed`, and `holiday`. |
...
| Persist only explicit overrides | Implemented | `dayOverrides` stores only non-default day edits. |
| Legacy status migration on load | Implemented | Older values are normalized in storage sanitization. |
...
| Visual distinction between statuses | Implemented | Worked, half-day, vacation, closed, holiday, and weekend have separate styles. |
...
| Dynamic year support beyond `2027` | Not implemented | Supported years are still hard-coded. |
...
3. Revisit the simplified status model only if users need finer-grained business categories than `vacation`, `closed`, and `holiday`.
